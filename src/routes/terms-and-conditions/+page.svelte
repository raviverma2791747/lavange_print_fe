<script>
  ...
  const initPolicy = async () => {
    ...
    if ($policy_store.terms_and_conditions) {
      loading = false;
      return;
    }
...
>
  ...
  {:else if $policy_store.terms_and_conditions}
    <Breadcrumb
      routes={[
        {
          name: "Home",
    ...
    <div class="font-semibold mb-4">
      Last Updated {formatDate(
        $policy_store.terms_and_conditions.updatedAt,
        DATE_FORMAT
      )}
    </div>
    <div class="dynamic-html">
      {@html $policy_store.terms_and_conditions.description}
    </div>
  {/if}
</div>
