<script>
  ...
  const initPolicy = async () => {
    ...
    if($policy_store.privacy_policy) {
      loading = false;
      return;
    }
    ...
    if (response.status === 200) {
      ...
      $policy_store.shipping_and_return_policy =  shipping_and_return_policy ?? null;
    }
    loading = false;
  };
...
>
  ...
  {:else if $policy_store.privacy_policy}
    <Breadcrumb
      routes={[
        {
          name: "Home",
    ...
    <div class="font-semibold mb-4">
      Last Updated {formatDate(
        $policy_store.privacy_policy.updatedAt,
        DATE_FORMAT
      )}
    </div>
    <div class="dynamic-html">
      {@html $policy_store.privacy_policy.description}
    </div>
  {/if}
</div>
